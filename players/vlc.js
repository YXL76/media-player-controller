...
module.exports = {
  ...
  _parseRequest: function (result) {
    for (var key in playerData) {
      ...
      if (previous.hasOwnProperty(key) && previous[key] === value) continue;
    ...
    if (result.currentplid > 0) previous.id = result.currentplid;

    if (
      !result.information ||
      !result.information.category ||
      !result.information.category.length ||
      (streams.count && streams.count === result.information.category.length)
    )
      return;
    ...
    for (var cat of result.information.category) {
      if (!cat["$"].name.startsWith("Stream")) continue;
    ...
    for (var type in streams) {
      ...
      if (streams[type].length > 1) streams[type].sort();
    }
  ...
  cleanup: function () {
    ...
    if (this._getDataTimeout) clearTimeout(this._getDataTimeout);
  },
  ...
  play: function (cb) {
    ...
    if (previous.pause) this.cyclePause(cb);
    else cb(null);
  },
  ...
  setRepeat: function (isEnabled, cb) {
    ...
    if ((isEnabled && previous.repeat) || (!isEnabled && !previous.repeat))
      return cb(null);
  ...
  _cycleStream: function (streamType) {
    if (!currStreams[streamType]) currStreams[streamType] = 0;
  ...
  addSubs: function (subsPath, cb) {
    ...
    if (!subsPath) return cb(new Error("No subtitles path specified"));
    ...
    const setAddedSubs = () => {
      ...
      if (!streams.subs.length) return cb(new Error("No subtitles tracks"));
