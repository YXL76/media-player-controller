...
const player = require("./players/vlc");
...
module.exports = class PlayerController extends PlayerSocket {
  ...
  launch(cb) {
    ...
    this.connectSocket(launchOpts, (err) => {
      ...
      if (this._parseSocketData && typeof this._parseSocketData === "function")
    ...
    const onSpawnError = (err) => {
      if (!this.process.pid) this.process = null;
  ...
  quit(cb) {
    ...
    if (!this.process) return cb(new Error("No open player process found!"));
  ...
  _getProbeTime(isPlaying, currTime, speed) {
    if (!isPlaying || !currTime) return 1000;

    if (currTime <= 0) return 200;

    if (this.prevProbeAt == currTime) {
      if (this.probeTime < 1100 / speed) this.probeTime += 10;
  ...
  _checkPlaybackStarted(event) {
    if (event.name !== "time-pos" || event.value < 1) return;
